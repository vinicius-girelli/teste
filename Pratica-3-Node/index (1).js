...
async function adicionarCarro(lista, carro) {
  ...
  return new Promise(async (resolve, reject) => {
    //retorne aqui um erro com o reject ou uma mensagem de sucesso com o resolve
    try {
      await fs.promises.writeFile('bd.json', json);
      resolve('Carro adicionado com sucesso!');
    } catch (erro) {
      reject('Erro ao adicionar carro: ' + erro.message);
    }
  });
}
...
//         reject(erro);
//       } else {
//         resolve(data); // Retorna os dados lidos do arquivo
//       }
//     })
//   });
// }
...
async function obterCarros() {
  return new Promise(async (resolve, reject) => {
    try {
      const data = await fs.promises.readFile('./bd.json', 'utf-8');
      resolve(data);
    } catch (erro) {
      reject(erro.message);
    }
